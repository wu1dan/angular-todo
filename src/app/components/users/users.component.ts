...
export class UsersComponent {
  ...
  logInUser(){
    if (this.logInLastNameControl.invalid){
      this.logInLastNameControl.setErrors({invalidLogInLastName : true})
    }
    if (this.logInNameControl.invalid || this.logInLastNameControl.invalid){
      return;
    }
    else{
      this.user = this.userService.logInUser(this.logInNameControl.value!, this.logInLastNameControl.value!)!;
  ...
  signUpUser(){
    if (this.signUpLastNameControl.invalid){
      this.signUpLastNameControl.setErrors({invalidSignUpLastName : true})
    }
    if (this.signUpNameControl.invalid || this.signUpLastNameControl.invalid){
      return;
    }
    else{
      this.userService.addUser(this.signUpNameControl.value!, this.signUpLastNameControl.value!)!;
      this.user = this.userService.logInUser(this.signUpNameControl.value!, this.signUpLastNameControl.value!)!;
      this.loggedIn = true;
      this.currentUserEvent.emit(this.user);
      this.modalRef.close();
    }
